...
struct CLine : CAsm
{
  int ln;		// nr wiersza w pliku �r�d�owym
...
struct CDebugLine : CAsm	// info dla symulatora o jednym wierszu �r�d�owym programu
{
...
public:
  ...
  {
    ASSERT( (bp & ~BPT_MASK) == 0 );	// niedozwolona kombinacja bit�w okre�laj�cych przerwanie
  ...
  void ClearAll()		// usuni�cie wszystkich przerwa�
...
class CDebugInfo : CAsm
{
  ...
  CDebugBreakpoints m_breakpoints;	// informacja o miejscach przerwa�
  CMapFile m_map_file;			// odwzorowania fazwy pliku �r�d�owego na 'fuid' i odwrotnie
...
public:
  ...
  { return fuid ? m_map_file.GetPath(fuid) : NULL; }	// nazwa (�cie�ka do) pliku
  void ResetFileMap()
  { m_map_file.Reset(); }
